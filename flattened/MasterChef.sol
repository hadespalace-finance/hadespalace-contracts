...
// File: contracts\libs\IHellFireReferral.sol



pragma solidity 0.6.12;

interface IHellFireReferral {
    /**
     * @dev Record referral.
     */
...
// File: contracts\HellFireToken.sol
...
// HellFireToken with Governance.
contract HellFireToken is BEP20 {
    ...
    // Min amount to liquify. (default 500 HELLFIREs)
    ...
    IUniswapV2Router02 public hellFireRouter;
    // The trading pair
    address public hellFirePair;
    // In swap and liquify
    ...
    event HellFireRouterUpdated(address indexed operator, address indexed router, address indexed pair);
    ...
    modifier antiWhale(address sender, address recipient, uint256 amount) {
        if (maxTransferAmount() > 0) {
            if (
                ...
                require(amount <= maxTransferAmount(), "HELLFIRE::antiWhale: Transfer amount exceeds the maxTransferAmount");
            }
        }
        _;
    }
    ...
    /**
     * @notice Constructs the HellFireToken contract.
     */
    constructor() public BEP20("Hell Fire", "HELLFIRE") {
    ...
    /// @dev overrides transfer function to meet tokenomics of HELLFIRE
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override antiWhale(sender, recipient, amount) {
        // swap and liquify
        if (
            swapAndLiquifyEnabled == true
            && _inSwapAndLiquify == false
            && address(hellFireRouter) != address(0)
            && hellFirePair != address(0)
            && sender != hellFirePair
            && sender != owner()
        ) {
            swapAndLiquify();
        ...
        if (recipient == BURN_ADDRESS || transferTaxRate == 0) {
            ...
            require(taxAmount == burnAmount + liquidityAmount, "HELLFIRE::transfer: Burn value invalid");

            // default 95% of transfer sent to recipient
            uint256 sendAmount = amount.sub(taxAmount);
            require(amount == sendAmount + taxAmount, "HELLFIRE::transfer: Tax value invalid");
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        ...
        path[0] = address(this);
        path[1] = hellFireRouter.WETH();

        _approve(address(this), address(hellFireRouter), tokenAmount);

        // make the swap
        hellFireRouter.swapExactTokensForETHSupportingFeeOnTransferTokens(
            tokenAmount,
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        // add the liquidity
        hellFireRouter.addLiquidityETH{value: ethAmount}(
            address(this),
            tokenAmount,
    ...
    // To receive BNB from hellFireRouter when swapping
    ...
    function updateTransferTaxRate(uint16 _transferTaxRate) public onlyOperator {
        require(_transferTaxRate <= MAXIMUM_TRANSFER_TAX_RATE, "HELLFIRE::updateTransferTaxRate: Transfer tax rate must not exceed the maximum rate.");
    ...
    function updateBurnRate(uint16 _burnRate) public onlyOperator {
        require(_burnRate <= 100, "HELLFIRE::updateBurnRate: Burn rate must not exceed the maximum rate.");
    ...
    function updateMaxTransferAmountRate(uint16 _maxTransferAmountRate) public onlyOperator {
        require(_maxTransferAmountRate <= 10000, "HELLFIRE::updateMaxTransferAmountRate: Max transfer amount rate must not exceed the maximum rate.");
     ...
     * Can only be called by the current operator.
     */
    function updateHellFireRouter(address _router) public onlyOperator {
        hellFireRouter = IUniswapV2Router02(_router);
        hellFirePair = IUniswapV2Factory(hellFireRouter.factory()).getPair(address(this), hellFireRouter.WETH());
        require(hellFirePair != address(0), "HELLFIRE::updateHellFireRouter: Invalid pair address.");
        emit HellFireRouterUpdated(msg.sender, address(hellFireRouter), hellFirePair);
    }
    ...
    function transferOperator(address newOperator) public onlyOperator {
        require(newOperator != address(0), "HELLFIRE::transferOperator: new operator is the zero address");
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "HELLFIRE::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "HELLFIRE::delegateBySig: invalid nonce");
        require(now <= expiry, "HELLFIRE::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "HELLFIRE::getPriorVotes: not yet determined");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying HELLFIREs (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "HELLFIRE::_writeCheckpoint: block number exceeds 32 bits");
